...
model = dict(
    ...
    twin=twin,
    twin_load_from='./workpairs/retinaMV2_iden/epoch_12.pth',
    neck=dict(
        type='FPN',
        in_channels=[24, 32, 96, 1280],
        out_channels=256,
...
test_cfg = dict(
    ...
    min_bbox_size=0,
    score_thr=0.05,
    nms=dict(type='nms', iou_thr=0.5, disable=True),
    max_per_img=-1)
# dataset settings
...
log_level = 'INFO'
work_dir = './workpairs/retinaMV2_256_twindirectabareshareall_skip'
load_from = None
resume_from = None
workflow = [('train', 1)]
