...
#include "fast_assemble_cuda_kernel.cuh"
...
__global__ void fast_assemble_forward(
...
template <typename scalar_t>
__global__ void fast_assemble_backward_Feat(
                                       ...
                                       cudaStream_t stream) 
{
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(feat.type(), "fast_assemble_forward", ([&] {

   fast_assemble_forward<scalar_t><<<totalBlocksCorr, threadsPerBlock, 0, stream>>> 
                                    ...
                                    cudaStream_t stream)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(feat.type(), "fast_assemble_backward_Feat", ([&] {
        fast_assemble_backward_Feat<scalar_t><<<totalBlocksCorr, threadsPerBlock, 0, stream>>> (
  ...
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    printf("error in fast_assemble_backward_cuda_kernel: %s\n", cudaGetErrorString(err));
    return 0;
  }
