...
_ext = cpp_extension.load(
    ...
    sources=[
        ...
        for srcfile in ["fast_assemble_cuda.cc", "fast_assemble_cuda_kernel.cu"]
    ],
    extra_cflags=cxx_args,
    extra_cuda_cflags=nvcc_args,
)

class FastAssembleFunction(Function):
...
class FastAssemble(Module):
    def __init__(self,
                 k):
        super(FastAssemble, self).__init__()
        self.k = k

    def forward(self, cur_prev_aff, feat):

        result = FastAssembleFunction.apply(
