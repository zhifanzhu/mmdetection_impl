...
int fast_assemble_forward_cuda(
        ...
        at::Tensor& masked_cpa)
{
  ...
  int success = fast_assemble_forward_cuda_kernel(
    output,
    cur_prev_aff,
    cur_prev_aff.size(1),
    feat,
...
int fast_assemble_backward_cuda(
        ...
        at::Tensor& masked_cpa)
{
  ...
  int success = fast_assemble_backward_cuda_kernel(gradOutput,
                                                gradOutput.size(0),
                                                gradOutput.size(1),
                                                gradOutput.size(2),
                                                gradOutput.size(3),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &fast_assemble_forward_cuda, "FastAssemble forward (CUDA)");
  m.def("backward", &fast_assemble_backward_cuda, "FastAssemble backward (CUDA)");
}
