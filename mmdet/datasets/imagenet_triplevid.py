...
class TripleVIDDataset(Dataset):
    ...
    def prepare_train_img(self, idx):
        ...
        offsets = np.random.choice(self.max_offset - self.min_offset + 1, 2, replace=False)
        bef_id = min(max(frame_ind + offsets[0], 0), num_frames - 1)
        aft_id = min(max(frame_ind + offsets[1], 0), num_frames - 1)
        if abs(bef_id - frame_ind) < abs(aft_id - frame_ind):
            near_id, far_id = bef_id, aft_id
        else:
            near_id, far_id = aft_id, bef_id

        near_ann_info = self.get_ann_info(idx, near_id)
        near_filename = osp.join(foldername, f"{near_id:06d}.JPEG")
        img_info['filename'] = near_filename
        near_results = dict(img_info=img_info, ann_info=near_ann_info)
        self.pre_pipeline(near_results)
        near_results = self.pipeline(near_results)

        far_ann_info = self.get_ann_info(idx, far_id)
        far_filename = osp.join(foldername, f"{far_id:06d}.JPEG")
        img_info['filename'] = far_filename
        far_results = dict(img_info=img_info, ann_info=far_ann_info)
        self.pre_pipeline(far_results)
        far_results = self.pipeline(far_results)

        results['near_img'] = near_results['img']
        results['far_img'] = far_results['img']
    ...
    def prepare_test_img(self, idx):
        ...
        TODO: we first consider training-style sampling
        """
        # img_info = self.img_infos[idx]
        # frame_ind = img_info['frame_ind']
