...
Possibly change:
    remove C5
    move out_from down one level
    dilation = 1 in atrous mode
'''
...
class SSDResNet_VOLATILE(ResNet):
    ...
    def __init__(self,
                 ...
                 fake_dilation_conv4=False,
                 use_dilation_conv5=True,
                 fake_dilation_conv5=False,
                 remove_conv5=False,
        ...
        if use_dilation_conv4:
            ...
            stride = 1
            dilation_first = 2 if not fake_dilation_conv4 else 1
            dilation_other = 1
        ...
        if use_dilation_conv5 and not remove_conv5:
            inplanes = self.feat_dim
            planes = 64 * 2 ** num_stages
            num_blocks = self.arch_settings[self.depth][1][num_stages]
            stride = 1
            dilation_first = 2 if not fake_dilation_conv5 else 1
            dilation_other = 1
            res_layer = _make_res_atrous_layer(
                self.block,
                inplanes,
