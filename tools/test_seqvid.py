...
def single_gpu_test(model, data_loader, num_evals, show=False):
    ...
    for i, data in enumerate(data_loader):
        if i == num_evals:
            break
        with torch.no_grad():
            result, _out_dict = model(return_loss=False, rescale=not show, **data)
...
def get_seq_gts(dataset):
    gt_bboxes = []
    gt_labels = []
    gt_ignore = []
    for i in range(len(dataset)):
        frame_ids = dataset.select_test_clip(i)
        anns = dataset.get_ann_info(i, frame_ids)
        for ann in anns:
            bboxes = ann['bboxes']
            labels = ann['labels']
